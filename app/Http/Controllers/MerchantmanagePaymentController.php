    ...
    public function merchantAccount(Request $request){
        ...
        foreach ($merchantaccounts as $account) {
            if($account->payment_method == 'bank'){
                $accounts.="<option value='".$account->id."'>".$account->holder_name.' | '.optional($account->bank)->name.' | '.$account->account_no.' | '.$account->branch_name."</option>";
            }elseif($account->payment_method == 'mobile'){
                $accounts.="<option value='".$account->id."'>".optional($account->mobileBank)->name.' | '.$account->mobile_no.'|'.$account->account_type."</option>";
