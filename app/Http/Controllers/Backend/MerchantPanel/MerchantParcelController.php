...
class MerchantParcelController extends Controller
{
    ...
    public function parcelImport(Request $request)
    {
        ...
        try {
            ...
            // Move uploaded file to temp dir with original extension
            $filename = $file->getClientOriginalName();
            $fullPath = $tempDir . DIRECTORY_SEPARATOR . $filename;
            $file->move($tempDir, $filename);

            // Import using the absolute path
            Excel::import(new ParcelImport, $fullPath);

            // Optional: delete temp file after import
            unlink($fullPath);
    ...
    {
        try {
            $data = $this->repo->parcelExport($request);

            if ($request->type === 'csv') {
                return Excel::download(
                    new MerchantParcelExport($data),
                    'Parcels-Export-' . Carbon::now()->format('d-m-Y_His') . '.csv',
                    ExcelType::CSV,
            ...
            return Excel::download(
                new MerchantParcelExport($data),
                'Parcels-Export-' . Carbon::now()->format('d-m-Y_His') . '.xlsx',
                ExcelType::XLSX
            );
