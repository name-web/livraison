...
class DeliverymanController extends Controller
{
    ...
    public function parcelSearchBarCodeFind($tracking_id)
    {
        $parcel = Parcel::where('tracking_id', $tracking_id)
            ->whereHas('parcelEvent', function ($eventquery) {
                $eventquery->where(function ($q) {
                    $q->where('delivery_man_id', Auth::user()->deliveryman->id)
                    ->orWhere('pickup_man_id', Auth::user()->deliveryman->id);
                });
            })
            ->first();

        if($parcel) {
            if (in_array($parcel->status, [ParcelStatus::DELIVERY_RE_SCHEDULE, ParcelStatus::PICKUP_ASSIGN, ParcelStatus::DELIVERY_MAN_ASSIGN])) {
                if ($parcel->status == ParcelStatus::DELIVERY_MAN_ASSIGN || $parcel->status == ParcelStatus::PICKUP_ASSIGN || $parcel->status == ParcelStatus::DELIVERY_RE_SCHEDULE) {
                    return $this->responseWithSuccess('Parcel', [
                        'parcel' => new ParcelResource($parcel)
                    ], 200);
                } else {
                    return $this->responseWithError('Parcel is Not Assign Delivery Man', [], 400);
                }
            }
            if ($parcel->status == ParcelStatus::DELIVERED || $parcel->status == ParcelStatus::PARTIAL_DELIVERED) {
                return $this->responseWithError('Parcel is delivered', [], 400);
            }
            if ($parcel->status == ParcelStatus::RETURN_TO_COURIER) {
                return $this->responseWithError('Parcel is Return To Courier', [], 400);
            }
        }else {
            return $this->responseWithError('Parcel is Not Assign Delivery Man', [], 400);
        }
        return $this->responseWithError('Parcel not found.', [], 400);
    }
