...
class PushNotificationService
{
    ...
    public function fcmSubscribe($request)
    {
        ...
        $topic = (notificationSettings()?->fcm_topic ?? env('FCM_TOPIC', '')) . '_' . str_replace(['@', '.', '+'], ['_', '_', ''], $request->topic);

        $headers = array(
            'Authorization: key=' . (notificationSettings()?->fcm_secret_key ?? env('FCM_SECRET_KEY')),
            'Content-Type: application/json'
        );
    ...
    {
        $deviceToken = $request->device_token;
        $topic = notificationSettings()?->fcm_topic ?? env('FCM_TOPIC');
    ...
    public function sendWebNotification($data,$notification,$type,$FcmToken)
    {
        ...
        if(!blank($FcmToken)) {
            ...
            $serverKey = notificationSettings()?->fcm_secret_key ?? env('FCM_SECRET_KEY');
            if ($notification) {
                $pushData = [
