...
class AppServiceProvider extends ServiceProvider
{
    ...
    /**
     * Merge mail settings stored in configs table into Laravel mail config at runtime.
     */
    protected function applyDatabaseMailSettings(): void
    {
        ...
        try {
            foreach ($map as $dbKey => $configKey) {
                $value = Config::where('key', $dbKey)->value('value');
                if (!blank($value)) {
                    config([$configKey => $value]);
                }
            }
        } catch (\Throwable $e) {
            // ignore during install or when database is unavailable
        }
    }
}
