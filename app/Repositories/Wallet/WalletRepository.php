...
class WalletRepository implements WalletInterface
{
    public function stats()
    {
        ...
        return [
            'balance' => (float) optional($user->merchant)->wallet_balance,
            'income' => (float) Wallet::where('user_id', $user->id)->where('type', WalletType::INCOME)->sum('amount'),
            'expense' => (float) Wallet::where('user_id', $user->id)->where('type', WalletType::EXPENSE)->sum('amount'),
            'pending' => Wallet::where('user_id', $user->id)->where('status', WalletStatus::PENDING)->count(),
            'approved' => Wallet::where('user_id', $user->id)->where('status', WalletStatus::APPROVED)->count(),
            'rejected' => Wallet::where('user_id', $user->id)->where('status', WalletStatus::REJECTED)->count(),
        ];
    }
