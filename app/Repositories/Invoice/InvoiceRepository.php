...
class InvoiceRepository implements InvoiceInterface
{
    ...
    {
        $query = Invoice::where('merchant_id', Auth::user()->merchant->id);

        return [
            'total' => (clone $query)->count(),
            'paid' => (clone $query)->where('status', InvoiceStatus::PAID)->count(),
            'unpaid' => (clone $query)->where('status', InvoiceStatus::UNPAID)->count(),
            'processing' => (clone $query)->where('status', InvoiceStatus::PROCESSING)->count(),
            'collection' => (float) (clone $query)->sum('cash_collection'),
            'charges' => (float) (clone $query)->sum('total_charge'),
            'payable' => (float) (clone $query)->sum('current_payable'),
        ];
    }
