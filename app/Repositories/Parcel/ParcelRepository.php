...
class ParcelRepository implements ParcelInterface {
    ...
    public function filter($request){
        ...
        return $query->where(function ($q) use ($request) {
            $this->applyParcelFilterConstraints($q, $request);
        })->paginate(10);
    }

    /**
     * Apply shared parcel list filter constraints (date, status, merchant, deliveryman, invoice).
     */
    private function applyParcelFilterConstraints($query, $request): void
    {
    ...
    public function deliverymanStatusParcel($status){
        ...
        if($status == ParcelStatus::DELIVERED):
            ...
            $statusArray = [$status];
            if($status == ParcelStatus::PICKUP_ASSIGN || $status == ParcelStatus::PICKUP_RE_SCHEDULE){
                $statusArray = [ParcelStatus::PICKUP_ASSIGN,ParcelStatus::PICKUP_RE_SCHEDULE];
            }

            return Parcel::orderByDesc('id')->with(['merchant'])->whereIn('status',$statusArray)->where(function($query) use ($status){
                $query->wherehas('parcelEvent',function($eventquery) use ($status){
                    if($status == ParcelStatus::PICKUP_ASSIGN || $status == ParcelStatus::PICKUP_RE_SCHEDULE){
                          $eventquery->where('pickup_man_id',Auth::user()->deliveryman->id);
                    }
                    else{
                        $eventquery->where('delivery_man_id',Auth::user()->deliveryman->id);
                    }
