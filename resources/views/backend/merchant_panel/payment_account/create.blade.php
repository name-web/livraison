...
<div class="container-fluid dashboard-content">
    ...
    <div class="wc-page-header">
        <div>
            <h1 class="wc-page-title">{{ __('merchant.create_account') }}</h1>
            <p class="wc-page-subtitle">{{ __('merchant.payment_accounts') }} · ajouter un compte bancaire ou mobile</p>
        </div>
    </div>
    ...
    <div class="wc-card !max-w-[960px]">
        <div class="wc-card-header">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="wc-card-title">{{ __('merchant.create_account') }}</h3>
        ...
        <div class="p-4 sm:p-6">
            <form action="{{route('payment.account.store')}}" method="POST" enctype="multipart/form-data" id="basicform">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {{-- bank --}}
                    <div class="wc-form-group m-0 bank"
                        @if(
                        $errors->has('holder_name') ||
                        $errors->has('branch_name') ||
                        $errors->has('bank_name') ||
                        ...
                        <input id="holder_name" type="text" name="holder_name" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.holder_name') }}" autocomplete="off" class="wc-input" value="{{old('holder_name')}}" required>
                    ...
                    <div class="wc-form-group m-0 bank"
                        ...
                        <input id="branch_name" type="text" name="branch_name" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.branch_name') }}" autocomplete="off" class="wc-input" value="{{old('branch_name')}}" required>
                    ...
                    @if(
                        ...
                        <input id="mobile_holder_name" type="text" name="mobile_holder_name" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.holder_name') }}" autocomplete="off" class="wc-input" value="{{old('mobile_holder_name')}}" required>
                        @error('mobile_holder_name')<small class="text-danger mt-1 d-block">{{ $message }}</small>@enderror
                    </div>
                    <div class="wc-form-group m-0 mobile" @if($errors->has('mobile_no')) @else style="display:none"  @endif>
                        <label class="wc-label" for="mobile_no">{{ __('merchant.mobile_no') }} <span class="text-wc-danger">*</span></label>
                        <input id="mobile_no" type="text" name="mobile_no" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.mobile_number') }}" autocomplete="off" class="wc-input" value="{{old('mobile_no')}}" required>
                    ...
                    <div class="wc-form-group m-0 bank"
                        ...
                        <input id="account_no" type="text" name="account_no" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.account_number') }}" autocomplete="off" class="wc-input" value="{{old('account_no')}}" required>
                        @error('account_no')<small class="text-danger mt-1 d-block">{{ $message }}</small>@enderror
                    </div>
                    <div class="wc-form-group m-0 bank md:col-span-2"
                        @if(
                        $errors->has('bank_name') ||
                        $errors->has('account_no') ||
                        $errors->has('routing_no')
                        ...
                        <input id="routing_no" type="text" name="routing_no" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.routing_number') }}" autocomplete="off" class="wc-input" value="{{old('routing_no')}}" required>
                ...
                <div class="bank-mobile" @if ($errors->any()) @else style="display: none" @endif>
