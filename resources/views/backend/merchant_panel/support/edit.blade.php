...
<div class="container-fluid dashboard-content">
    ...
    <div class="wc-page-header">
        <div>
            <h1 class="wc-page-title">{{ __('support.support_edit') }}</h1>
            <p class="wc-page-subtitle">{{ __('support.supprot') }} · modifier le ticket</p>
        </div>
    </div>
    ...
    <div class="wc-card !max-w-[960px]">
        <div class="wc-card-header">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="wc-card-title">{{ __('support.support_edit') }}</h3>
            ...
            <form action="{{ route('merchant-panel.support.update', ['id' => $singleSupport->id]) }}" method="POST" enctype="multipart/form-data" id="basicform">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="wc-form-group m-0">
                        ...
                        <input type="date" name="date" class="wc-input" value="{{ old('date', $singleSupport->date) }}">
                    </div>
                    <div class="wc-form-group m-0">
                        <label class="wc-label" for="service">{{ __('support.service') }} <span class="text-wc-danger">*</span></label>
                        <input id="service" type="text" name="service" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.service') }}" autocomplete="off" class="wc-input" value="{{ old('service', $singleSupport->service) }}" required>
                    ...
                    <div class="wc-form-group m-0">
                        ...
                        <select name="priority" class="wc-select @error('priority') is-invalid @enderror">
                            ...
                            <option value="low"{{ old('priority', $singleSupport->priority) == 'low' ? 'selected' : '' }}>Low</option>
                            <option value="medium"{{ old('priority', $singleSupport->priority) == 'medium' ? 'selected' : '' }}>Medium</option>
                            <option value="high"{{ old('priority', $singleSupport->priority) == 'high' ? 'selected' : '' }}>High</option>
                        </select>
                    ...
                    <div class="wc-form-group m-0">
                        ...
                        <input id="subject" type="text" name="subject" data-parsley-trigger="change" placeholder="{{ __('merchantPlaceholder.subject') }}" autocomplete="off" class="wc-input" value="{{ old('subject', $singleSupport->subject) }}" required>
                    ...
                    <div class="wc-form-group m-0 md:col-span-2">
                        ...
                        <textarea class="wc-input !h-auto resize-y" name="description" rows="5" id="description" placeholder="Enter Description">{{ $singleSupport->description }}</textarea>
                    </div>
                </div>
