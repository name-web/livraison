...
<section class="t5-page-hero">
    <div class="container text-center">
        <h1 class="t5-page-title">{{ __('levels.parcel_tracking') }}</h1>
    </div>
</section>
<section class="t5-tracking-page t5-page-content">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 mx-auto">
                <form action="{{ route('tracking.index') }}" method="GET">
                    <div class="t5-tracking-form d-flex flex-column flex-sm-row gap-2 mb-4">
                        <input type="text" class="form-control flex-grow-1" placeholder="{{ __('levels.enter_tracking_id') }}" name="tracking_id" value="{{ $request->tracking_id }}">
                        <button type="submit" class="btn btn-track"><i class="fa fa-search me-2"></i>{{ __('levels.track_now') }}</button>
                    </div>
                </form>
                @if(!empty($request->tracking_id))
                    <h3 class="text-center fw-bold mb-4">{{ __('levels.parcel_tracking_no') }}: <span style="color:var(--t5-primary)">#{{ $request->tracking_id }}</span></h3>
                @endif
            </div>
        </div> 
